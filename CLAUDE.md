...
- ファイル: `docs/oidf/<wg-id>/<year>-<mm>.md`（例: `docs/oidf/fapi/2026-03.md`）
  - WG/CG ごとにディレクトリが分かれ、URL も `/oidf/<wg-id>/<year>-<mm>` となる
  - `<mm>` は 2 桁ゼロパディング（`01`〜`12`）
- 執筆: `/oidf` スキル（過去月の遡及執筆が主目的）
- ファイル名は **対象月**（執筆日ではない）。遡及執筆でも WG 内サイドバーの並び（月降順）を安定させるため
- 執筆日は本文冒頭に明記する
- フロントマターは `title` のみ
- 各 WG/CG ディレクトリには `index.md` を置き、概要と外部リンクを記載する（`/review` 対象外）

### サイドバー表示

- `/articles/` サイドバー: `docs/articles/` の自由トピック記事をファイル名降順（執筆日の新しい順）に表示
- `/oidf/` サイドバー: WG/CG ごとに折り畳みグループを構成し、各グループ内で月次レポートを対象月降順に表示
- `/oidf/` ランディングページから全 WG/CG の一覧にアクセスできる。トップページおよびナビゲーションからも導線あり
