...
2025〜2026年にかけて、選択的開示をめぐる主要な標準化が相次いで成熟期を迎えている。本稿ではその代表的な三つのアプローチ――**SD-JWT**・**BBS署名**・**mDoc/mDL（ISO 18013-5）**――の仕組みと採用状況を整理する。
...
EU の **eIDAS 2.0** では、Architecture Reference Framework（ARF）において SD-JWT VC と ISO 18013-5（mdoc）の**両フォーマット**が PID プロバイダー・ウォレット・検証者に対して必須として規定されており、欧州デジタルアイデンティティウォレット（EUDI ウォレット）の中核を担っている。
...
## BBS署名：プライバシー最強のアプローチ

### 概要

**BBS署名**（BBS Signatures）は、マルチメッセージ署名とゼロ知識証明を組み合わせた暗号方式だ（旧称「BBS+」としても広く知られる）。Identity Foundation の BBS Signature 仕様（draft-irtf-cfrg-bbs-signatures）として標準化が進められており、W3C は **Data Integrity BBS Cryptosuites v1.0** として Candidate Recommendation Draft を公開している。
...
2025〜2026年にかけて ISO 18013-5 の改訂版が DIS（Draft International Standard / 国際規格案）ステージに入っており、ゼロ知識証明の組み込みも将来の機能として検討されている。
...
| 述語証明          | ✗             | ✓              | ✗                 |
| 既存JWTとの親和性 | ✓             | ✗              | ✗                 |
| 主要採用規制      | eIDAS 2.0     | -              | eIDAS 2.0/各国mDL |
| 計算コスト        | 低            | 高             | 中                |
