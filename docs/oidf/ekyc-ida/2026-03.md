...
GitHub 側では 3月分の有意な新規 issue / PR は見当たらず、議論の中心は引き続き Bitbucket のイシュートラッカーとメーリングリストにある。Bitbucket 側では、過去から継続している `#1194 eKYC eIDAS bridge` など、eIDAS（欧州デジタルID枠組み）との接続性に関する議論が引き続き進行中と見られる。
