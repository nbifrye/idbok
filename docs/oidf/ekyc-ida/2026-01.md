...
議事録は Bitbucket wiki に公開される慣例だが、本記事執筆時点（2026-04-19）においてアクセス不可であり、以下はメーリングリストの議題メールから再構成したものである。
...
- Juliana Cafik による KYC メタデータのプレゼンテーション
...
1 月 7 日と同じ常設議題を引き継いだ。ISO PAS 向け PR の第二陣（PR#267・PR#271）がマージされた。Authority チャーター・AIIM 連携・Profiles & Registries の各検討が継続された。
