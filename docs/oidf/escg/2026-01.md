...
### OpenID for Verifiable Credentials の形式的セキュリティ検証

University of Stuttgart の Information Security 研究所が Web Infrastructure Model (WIM) を用いて OpenID4VCI v1.1（IAE 含む）の形式的セキュリティ証明を完了し、1月22日に7件の issue として DCP WG へ提供された。結果は OIDF ブログでも公開されており（openid.net/formal-security-analysis-openid-verifiable-credentials/）、ESCG が推進するエコシステムの技術的信頼性向上に寄与する成果である。
...
2026年1月、OpenID Foundation は EU デジタルアイデンティティウォレット（EUDIW）の実装者向けに集約型リソースハブを立ち上げた。このハブは ESCG のミッションである「エコシステムオーナーへの実装支援」の一環として位置づけられており、EU（eIDAS 2.0）を含む38法域での OIDF 仕様採用を促進するための技術ガイダンス・規格文書へのアクセスを一元化するものである。
...
| EUDIW リソースハブ             | https://openid.net/openid-foundation-launches-eudiw-resource-hub/                          | EU デジタルアイデンティティウォレット実装者向けリソース集約    |
| HAIP 1.0 相互運用テスト結果    | https://openid.net/haip-1-0-openid4vp-1-0-achieve-98-in-oidf-interop-testing/              | Wallet/Verifier ペアで98%の合格率                              |
| 形式的セキュリティ検証発表     | https://openid.net/formal-security-analysis-openid-verifiable-credentials/                 | Stuttgart 大学による OpenID4VCI の形式検証結果                 |
