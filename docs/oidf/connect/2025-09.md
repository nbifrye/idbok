...
AB/Connect WG（Artifact Binding / Connect Working Group、通称「Connect WG」）は、OpenID Connect Core を中心とする OIDC 系仕様、OpenID Federation、各種拡張仕様（Native SSO、RP Metadata Choices ほか）を策定する OpenID Foundation の中核 WG である。共同議長は Michael Jones（self-issued）、Nat Sakimura（NAT Consulting）、John Bradley の3名で、コール議事録は ML 上で公開される運用（議事録自体は non-public）。
