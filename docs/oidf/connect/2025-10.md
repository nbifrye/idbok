...
AB/Connect Working Group（旧 Artifact Binding WG と OpenID Connect WG の合流組織、以下「Connect WG」）は、OpenID Connect 仕様ファミリーおよび OpenID Federation の策定を担う WG である。議長は Mike Jones（エディタとしても活動）、副議長に Nat Sakimura。定例コールは毎週木曜 UTC 16:00（米国側 Atlantic 時刻）に実施される。
...
- **OpenID Federation Draft 44 公開（10月20日）**: IETF 124 submission cutoff に合わせたリリース。**スウェーデン政府ユースケースに動機づけられた機能群**（extension points）と、**Trust Mark Status レスポンスの署名必須化** を含む
...
### OpenID Federation Draft 44（2025年10月20日公開）

- **告知**: Mike Jones が [OpenID Federation draft 44 Incorporating Features Motivated by Swedish Government Use Cases](https://lists.openid.net/pipermail/openid-specs-ab/2025-October/011079.html) として投稿
- **主要変更点**:
  - **Trust Mark Status レスポンスの署名必須化**: Trust Mark の状態（active / revoked 等）を返すレスポンスを署名付き JWT にすることを規定
  ...
  - 既存機能の説明改善（editorial）
- IETF 124 Montreal の submission cutoff（10月20日）に間に合わせるリリースであり、翌11月の WGLC 対象バージョンへの前段階
...
[OpenID Federation draft 44 Incorporating Features Motivated by Swedish Government Use Cases](https://lists.openid.net/pipermail/openid-specs-ab/2025-October/011079.html) - 2025-10-23（改訂通知） / Michael Jones / 2通
