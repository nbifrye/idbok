...
また1月2日にはエディタの Mike Jones が OpenID Federation 仕様を議定書非依存部分と議定書依存部分に分割した **1.1 系エディタ草案** を公開し、翌週の1月20日には OpenID Federation 1.0 最終仕様承認投票の予告が発出された。さらに同日、レビュー期間中に寄せられたフィードバックを反映したドラフト -47 も公開されており、最終仕様化に向けた作業が佳境を迎えていた。
...
| OpenID Federation for OpenID Connect 1.1 | プロトコル依存部分（OpenID Connect 固有のプロファイル）                         |
...
| カテゴリ                        | 当選者                                     | 任期 |
| ------------------------------- | ------------------------------------------ | ---- |
| Corporate Representative（1席） | Mark Verstege                              | 1年  |
