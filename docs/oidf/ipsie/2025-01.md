...
- 1/15 に Okta Security Blog で Todd McKinnon CEO 名義の [Raising the Bar for the Industry with IPSIE](https://sec.okta.com/articles/2025/01/raising-the-bar-for-the-industry-with-ipsie/) が公開。WG 結成後の業界向け広報の中心的な投稿となる
...
PR #34 では Tim Cappalli (Microsoft) の協力のもと NIST SP 800-63B 由来の MFA 定義が組み込まれた。PR #35 では phishing-resistant authentication の用語が追加され、ttripp と aaronpk のレビューを経て 1/17 までに合意形成（マージ自体は 2/4）。
...
- [PR #42 "Added term for tenant"](https://github.com/openid/ipsie/pull/42) (ttripp, 1/23 起票, 2/4 マージ) — tenant 用語の追加。Dean のコメントを受けて 1/23 中に修正
...
これらが [PR #41 "User/Group lifecycle management"](https://github.com/openid/ipsie/pull/41) として 1/29 にマージされた。PR #41 は **(1) Entitlements を IdP 管理可否で 3 段階 → 2 段階に縮約、(2) JIT 時にアプリは「least privileges possible」でアカウントを作成するという推奨の追加、(3) `IdP` / `RP` 呼称を `Identity Service` / `Application` に置き換える** という 3 つの構造変化を一度に持ち込んだ。
...
- **「IdP」呼称の放棄**: エンタープライズが「IdP」と呼ばれる単一のソフトウェアではなく、認証・プロビジョニング・エンタイトルメントに対し **異なるツールの組み合わせ** を使っているという現実認識が共有された。Shannon Roddy の発言が転換点として議事録に明記:
  ...
  > "My IdP does JIT. I can also provide group info. But I won't preprovision." — Shannon Roddy（議事録 2025-01-28 より）
...
- 主要な変更と議論:
  - **Entitlements を 3 段階から 2 段階に縮約**: 「IdP がエンタイトルメント管理を一切できないアプリは IPSIE 適合と認めるべきではない」というレビュアの主張を受け、Entitlements の最下位レベルを実質的に排除する形で 2 段階に整理
  - **JIT 時の least-privilege 推奨**: 「JIT リクエスト時にアプリが過剰な権限を持ったアカウントを生成するセキュリティリスク」を踏まえ、`applications should create accounts with least privileges possible`（パーミッションレベルが明示されない場合）という推奨文を導入
  - **`IdP` → `Identity Service`, `RP` → `Application` への呼称変更**: 1/28 コール直後の Dean のコミット (`38e2a5f`) で同 PR 内に取り込まれた
