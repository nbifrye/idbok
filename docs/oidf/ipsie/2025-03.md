...
2025 年 3 月は **WG 発足から約 5 か月、レベル定義（SL1/SL2/SL3、IL1/IL2/IL3）の枠組み合意が直前の 2/25 コールで成立した直後** にあたる。月初の 3/4 コールでは「最初のプロトコル仕様を SL1 / IL1 に絞って書き始める」方針と「年末 12 月の Gartner IAM での interop デモを WG 全体のゴールに据える」スケジュール感が共有された。これを受けて 3 月 7 日に Aaron Parecki が **OpenID Connect IPSIE SL1 Profile の Editor's Draft 初版** を `aaronpk/ipsie-openid-sl1` リポジトリに投入し、ML へ周知。同 7 日には Identity Defined Security Alliance (IDSA) との共催で IPSIE を題材にしたウェビナーが開催され、登録者 300 名超を集めて IPSIE の存在を業界に広く周知する月にもなった。
...
- 同 3/7、Aaron / Dean / George / Gail と Jeff Reich (IDSA) によるウェビナー「Securing the Future of Identity with IPSIE – A New Industry Standard」開催（[告知記事](https://openid.net/webinar-on-ipsie-secures-more-than-300-registrations/)）。登録者 300 名超
- SL1 Editor's Draft 初版に対し、`openid/ipsie` 親リポジトリで **6 件の新規 SL1 関連 issue（#58, #59, #60, #61, #62, #63）** と SL3 関連 1 件（#56）、SL3/SSF 関連 1 件（#66）が起票され、いずれも実質的な議論を伴った
...
3 月最大の編集作業は、Aaron Parecki が個人リポジトリ `aaronpk/ipsie-openid-sl1` 上で SL1 OIDC プロファイルの Editor's Draft 初版を立ち上げたことである。3/7 に集中して 20 コミットが投入され、`martinthomson/i-d-template` を用いた Internet-Draft 形式（IETF kramdown）で初版が形成された。代表的なコミット履歴:
...
### 3/7 IDSA × OpenID Foundation ウェビナー「Securing the Future of Identity with IPSIE – A New Industry Standard」

3 月最大の対外イベント。Identity Defined Security Alliance (IDSA) と OpenID Foundation の共催で、IPSIE WG を業界に紹介するオンライン webinar が 3/7 に開催された。OpenID Foundation のアナウンス記事 [Webinar about IPSIE secures more than 300 registrations](https://openid.net/webinar-on-ipsie-secures-more-than-300-registrations/) は、登録者が **300 名超** に達したことを伝えており、IPSIE が業界の高い関心を集めていることを示している。
