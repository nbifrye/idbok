...
`openid-specs-authzen` ML の 2026年3月アーカイブには全16通が投稿された。主なスレッドは以下のとおり（投稿日順）。

- **「Proposed agenda for March 5th, 2026」「Proposed agenda for March 12th, 2026」「Proposed agenda for March 26th, 2026」**（Alex Olivier） — 定例コールの事前アジェンダ共有
- **「Thursday Meeting agenda + Pull requests」**（Alex Babeanu） — 当日のレビュー PR 整理
- **「Fwd: [modelcontextprotocol/modelcontextprotocol] Spec Proposal: A Gateway-Based Authorization Model」**（David Brossard） — MCP 側で提案された Gateway ベース認可モデルの DCP への転送。COAZ プロファイル議論との接続点として注目された
- **「Call notes」**（Atul Tulshibagwale） — 定例コールの議事メモ共有
- **「Updated COAZ proposal」**（Atul Tulshibagwale、返信: Julio Auto De Medeiros、Alex Babeanu、Martin Besozzi） — COAZ プロファイル改訂版をめぐる議論スレッド
- **「Obligations and Advices」**（Alex Babeanu、返信: Omri Gazitt） — Obligations／Advices（XACML 系用語）の AuthZEN への持ち込み方に関する議論
- **「COAZ deck」**（Atul Tulshibagwale） — COAZ 紹介スライドの共有

ML 側では MCP 連携（Gateway Authorization Model）と COAZ プロファイル、Obligations / Advices という v1.1 向け主要議題が、短いながら複数参加者のやり取りとして展開されている。
...
- [openid-specs-authzen メーリングリスト アーカイブ](https://lists.openid.net/pipermail/openid-specs-authzen/) - ML アーカイブインデックス
