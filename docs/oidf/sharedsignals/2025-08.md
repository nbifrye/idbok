...
レビューでは tulshi (Atul Tulshibagwale) が承認、appsdesh (Apoorva Deshpande) は **バージョン番号付与方針への懸念** をレビューに残した。これは「過去ドラフト版から飛んでバージョン番号がジャンプしている（SSF/CAEP は v3 から v5、RISC は v1 から v3）」点に関する確認で、8 月 5 日定例コール (§3.1) でも改めて議論されることになる。
...
- **バージョン番号ジャンプへの問い**: Apoorva Deshpande が「より高いバージョン番号を維持する決定を取り下げるのか」と質問。Shayne Miel は「新しいドラフトに移行するタイミングで（バージョン表記を）更新する」と説明。SSF/CAEP は v3 から v5 へ、RISC は v1 から v3 へ番号がジャンプしている経緯を整理。
...
- **Issue #288: aliases 形式での Subject 追加・削除のセマンティクス**: 8 月 26 日に TakahikoKawasaki が起票したばかりの [Issue #288](https://github.com/openid/sharedsignals/issues/288) を題材に議論が展開された。論点は以下:
  ...
  Shayne Miel: 「複数のエイリアス形式を追加した後で一部のみを削除する場合、残りは維持されるべき」(議事録 2025-08-26 より)

  George Fletcher: 「**SSF はシグナルについての仕様であり、エンティティモデルへの依存性を過度に要求すべきではない**」(議事録 2025-08-26 より)

  Atul Tulshibagwale は逆に「同一エンティティを表すエイリアスならば、いずれかの識別子で削除すれば全体を削除すべき」という立場で、両者の議論が並列のまま継続。

- **議論内容を Issue にまとめる**: Sean O'Dell が議論内容を Issue #288 に追記する役割を担うことで合意（議事録 2025-08-26 より）。
