...
人事面では、共同議長の一角を担っていた Shayne Miel（Cisco）が他業務に専念するため退任を表明し、後任候補として **Mike Kiser（SailPoint）が共同議長に指名・承認** された（9 月 23 日コール）。GitHub では 3 件の Issue（#293, #294, #295）が新規起票され、いずれも SSF 1.0 Final で残された設計課題を vFuture へ繰り上げる動きとなった。
...
- **Interop Spec 完了目標の延期**: 9 月末完了目標を **10 月末** に変更。Vatsal Gupta（Apple）が Receiver 要件の整理に協力する旨を表明。
...
- **Google の SSF クローズドベータ**: Tushar Raibhandare（Google）から「Google が Shared Signals Framework サポートのクローズドベータを開始した」旨の告知。Google Cloud インスタンスを持つ参加者が Transmitter として参加可能。**現状はサードパーティ IdP セッションの失効に対応するが、GCP 自体のセッション失効には直接対応していない** という制約も共有された。
...
| [Week-of-Mon-20250929](https://lists.openid.net/pipermail/openid-specs-risc/Week-of-Mon-20250929/thread.html) |      6 | 9 月 30 日コールキャンセル通知 2 件、GitHub Issue #296 起票通知群                                  |
...
- **Issue 新規起票**: 3 件（#293, #294, #295、いずれも 9 月 16 日）+ #296（9 月 29 日）
...
### 5.4 [openid/sharedsignals#292](https://github.com/openid/sharedsignals/pull/292) — Align CAEP Interop profile with finalized specs
...
- author: ashish1294
- 起票: 2025-09-29
- 内容: 9 月最終日に新規起票された Issue。Transmitter と Receiver 間で共有される各シグナルに **一意識別子を付与し、責任連鎖（chain-of-responsibility）を確立すべき**との提案。Receiver が「なぜそのアクションが行われたか」の監査ログを残し、エンドユーザが Transmitter のシステムまで遡って調査できるようにする狙い。A → B → C のような複数組織を跨いだシグナルチェーンの追跡もユースケースとして挙げられた。本 Issue は翌月の 10 月 7 日コール中にクローズされ、その議論が翌月の Identifier 整合性議論（Issue #297 等）に繋がる。
