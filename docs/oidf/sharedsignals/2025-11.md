...
- **P0/P1/P2 ラベルによる優先度付け**: GitHub に `p0`〜`p2` ラベルを追加し、vFuture Issue にラベル付与を開始。**P0 として 2 件** が特定された:
  1. **JWKS ベース受信者認証**（11 月 4 日コールで提起された OAuth 代替策）
  2. **受信者向け Well-Known 設定**（Issue #298、"receiver capability" と呼称。送信者が企業ネットワーク内の受信者能力を発見可能にする仕様）
     Well-Known 設定の議論では、マルチテナント環境での受信者発見とテナント情報の露出に関するセキュリティ懸念も挙げられた。
...
- **vFuture Issue の P0 項目の設計議論**: JWKS ベース受信者認証、および受信者 Well-Known 設定（Issue #298, #304）の設計詳細化。12 月以降の定例コールで継続議論
