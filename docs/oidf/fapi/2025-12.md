...
議事録は Bitbucket wiki（`FAPI_Meeting_Notes_2025`）に掲載されているが、本稿執筆時点では公開ページへのアクセスができなかったため、開催されたコールの詳細な議事内容は確認できなかった。
...
FAPI WG の主要なトラッカーは Bitbucket（`bitbucket.org/openid/fapi`）であり、2025 年 12 月の具体的な issue/PR の議論は本稿執筆時点では確認できなかった。

上述のとおり、11 月に提起された Issue #843〜#846 が 12 月に持ち越されており、IANA 登録や ISO 対応の編集作業が進んでいたと推察される。
...
- [Bitbucket FAPI リポジトリ](https://bitbucket.org/openid/fapi/) – Issue トラッカー（本稿執筆時点でアクセス不可）
