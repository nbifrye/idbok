...
| クレーム | 必須 | 内容                                                                                                                                                          |
| -------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `iss`    | 必須 | JWT を発行したエンティティを一意に識別する文字列                                                                                                              |
| `sub`    | 必須 | JWT の主体。Authorization Grant ではリソースオーナーを、Client Authentication ではクライアント自身 (通常 `client_id` と一致) を示す                           |
| `aud`    | 必須 | JWT の意図された対象者。認可サーバーを識別する値を含めること。仕様上は認可サーバーのトークンエンドポイント URL を `aud` の値として用いてもよい (MAY) とされる |
...
RFC 7523 セクション 3 が定める JWT への要件は次の通り (原文は番号付きリストで規定される)。

1. `iss` クレームを必ず含めること。値は JWT を発行したエンティティの一意識別子
2. `sub` クレームを必ず含めること。JWT の主体を識別する
3. 認可サーバーを意図された対象者として識別する値を `aud` クレームに含めること。複数の値を指定してもよいが、認可サーバーは自身が `aud` に含まれていない JWT を拒否しなければならない
4. `exp` クレームを必ず含めること。認可サーバーは現在時刻と比較して期限切れを拒否する
5. 任意で `nbf` クレームを含めてよい (それ以前の処理を拒否)
6. 任意で `iat` クレームを含めてよい
7. 任意で `jti` クレームを含めてよい。認可サーバーは使用済み `jti` を一定期間保持することでリプレイを防止する**ことができる** (MAY)
8. その他のクレームを含めても構わない
9. JWT は発行者の鍵によりデジタル署名されるか MAC が付与されなければならない。`alg=none` (Unsecured JWT) は許されない
10. 認可サーバーは JWT 仕様に照らして無効な JWT および無効な署名/MAC を持つ JWT を拒否しなければならない
...
- Refresh Token の扱いは RFC 7523 自体では明示的に規定されておらず、上位の RFC 7521 (Assertion Framework) の規定 ("典型的にはアサーション提示時にリフレッシュトークンは発行されない") に従う

### 5.3 Client Authentication 固有の意味付け

- `iss`: クライアント識別子 (`client_id`)
- `sub`: クライアント識別子 (通常 `iss` と同一値)
- `aud`: 認可サーバーを識別する値。RFC 7523 では認可サーバーのトークンエンドポイント URL を `aud` の値として用いてもよい (MAY) と規定されており、実運用ではこの値を採用するのが一般的である。Issuer 識別子等の他の値を許容するかは認可サーバーの裁量
- `jti` の使用は仕様上は任意 (MAY) だが、リプレイ攻撃防止のために設定し、認可サーバーが一定期間記録することが望ましい
