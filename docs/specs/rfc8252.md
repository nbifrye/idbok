...
### 6.7 Redirect URI の完全一致比較 (Sec 8.4)

Authorization Server は登録済み Redirect URI と着信した Redirect URI を完全一致で比較し、一致しない認可リクエストを拒否しなければならない (Loopback のポート番号成分のみ例外として許容)。部分一致やワイルドカードは認可コード横取りの足がかりとなるため避ける。あわせて、Private-Use URI Scheme については、ピリオドを含まない (= 逆ドメイン記法でない) スキームは拒否することが推奨される。

### 6.8 Authorization Server 混同攻撃の緩和 (Sec 8.10)

同一アプリが複数の Authorization Server を利用する場合、悪意ある (あるいは侵害された) AS が別の AS の認可レスポンスを横取りすることを防ぐため、各 AS ごとに一意な Redirect URI を用いる (例: パス成分を AS ごとに変える) ことが要求される。受信した Redirect URI が送信した認可リクエストのものと一致しない場合は、認可レスポンスを拒否する。

### 6.9 Embedded User-Agent の禁止 (Sec 8.12)
