...
### 5.3 ACK と setErrs (Section 2.2 / 2.4)
...
### 8.1 署名付き SET による真正性確認 (Section 4.1)

RFC 8417 / RFC 7515 に従い、JWS で署名された SET を用いることで、Recipient は SET Issuer が actionable な SET を提供する権限を持つことを検証できる。

### 8.2 HTTP に関する考慮事項 (Section 4.2)

本プロトコルは HTTP に依存するため、RFC 7230 Section 9 をはじめとする HTTP 系仕様のセキュリティ考慮事項が適用される。

### 8.3 SET の機密性と TLS (Section 4.3)

Poll 配信におけるエンドポイントは最低 TLS 1.2 をサポートしなければならず、より新しい版 (本仕様発行時点では TLS 1.3, RFC 8446) のサポートが望ましい。クライアントは DNS-ID (RFC 6125) または DANE (RFC 6698) を用いた TLS サーバー証明書検証を行わなければならない。さらに RFC 7525 (TLS / DTLS の安全な利用に関する推奨) に従うことが要求される。SET が個人識別情報 (PII) 等の機微情報を含む場合、Transmitter および Recipient はその機密性を保護しなければならず、TLS だけで不十分な場合は JWE (RFC 7516) による暗号化を併用する。

### 8.4 アクセストークンの取り扱い (Section 4.4)

HTTP 認証に OAuth アクセストークン (RFC 6749) を用いる場合、RFC 7521 Section 8 の脅威と対策を考慮しなければならない。Bearer Token (RFC 6750) は TLS 上で送受信することで傍受を防ぐ。Bearer Token は直接的または間接的 (検証サービスでの確認等) に有効期限を判定できる短命なものとすべきである (4.4.1 Bearer Token Considerations)。

### 8.5 配信の堅牢性とリプレイ

本プロトコルは構造的に at-least-once であるため、Recipient は同じ SET を複数回受け取りうる。SET の `jti` を用いた重複排除は受信側実装が考慮すべき事項である。また、未 ACK の SET について Transmitter がどこまで保持するかは本仕様では規定されておらず、out-of-band の合意に委ねられる。

### 8.6 プライバシー (Section 5)
...
- **RFC 6749 (OAuth 2.0) / RFC 6750 (Bearer Token Usage) / RFC 7521 (Assertion Framework)**: アクセストークンを使う場合の関連仕様 (本仕様 Section 4.4 で参照)
- **RFC 8705 (Mutual-TLS Client Authentication)**: mTLS を OAuth クライアント認証に用いる際の参照仕様 (本仕様自体は具体的なプロファイルを指定しない)
