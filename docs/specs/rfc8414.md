...
クライアントはレスポンスに含まれる `issuer` クレームの値が、メタデータ取得に使用した URL のプレフィックスと一致することを検証しなければならない。一致しない場合はそのレスポンスを使用してはならない。
...
| フィールド               | 条件                                                          |
| ------------------------ | ------------------------------------------------------------- |
| `authorization_endpoint` | Authorization Code フローなど implicit 以外をサポートする場合 |
...
flowchart TD
    ...
    D --> E[JWT 署名を検証]
    E --> F[JWT ペイロードのクレームを使用]
    C -->|No| G[JSON クレームをそのまま使用]
```

署名付きメタデータを使用する場合、クライアントは JWT の署名を検証し、`issuer` クレームが期待する値と一致することを確認する必要がある。
