...
```mermaid
sequenceDiagram
    participant OP as OpenID Provider (AS)
    participant RP as クライアント/RP
    participant JWKS as JWKS Endpoint

    OP->>OP: IDトークンを秘密鍵で署名して発行
    OP->>RP: IDトークン発行

    RP->>JWKS: 公開鍵を取得（GET /.well-known/jwks.json）
    JWKS->>RP: JWK Set（公開鍵群）
    RP->>RP: kidで鍵を選択し署名を検証
```
...
JWK Set内の異なる鍵には通常、一意の`kid`を使用すべきである（SHOULD）。ただし、異なる`kty`を持つ等価な代替鍵については、同じ`kid`を使用できる例外がある。
