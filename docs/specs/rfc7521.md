...
RFC 7521 はアサーション形式そのものを規定せず、抽象的なメッセージフロー、必要なパラメータ、アサーションに含まれるべき情報、検証要件、セキュリティ考慮事項を定義する。具体的なアサーション形式は、SAML 2.0 を扱う [RFC 7522](https://datatracker.ietf.org/doc/html/rfc7522) や JWT を扱う [RFC 7523](./rfc7523) など、コンパニオン仕様によって定義される。
...
- **Assertion ID**: 一意な識別子 (nonce)。リプレイ防止のため認可サーバーが追跡するのに利用できる
...
5. クロックスキューを許容しつつ、有効期限を過ぎたアサーションは拒否する
...
RFC 7521 自体は両概念を区別したうえで、本仕様で定義するプロトコルパラメータと処理規則は Bearer アサーションを前提としており、Holder-of-Key アサーションの利用には別途追加の仕組みが必要であると明記している (Section 3)。コンパニオン仕様である RFC 7522/7523 もいずれも **Bearer** 形式に特化したプロファイルである。
...
RFC 7521 は、認可サーバーが発行するアクセストークンの有効期間を、アサーションの有効期間を大きく超えて延長すべきではないと注意喚起している。長寿命のアサーションは Bearer 性質と相まって深刻な被害につながるため、可能な限り短い有効期限を設定することが推奨される。
