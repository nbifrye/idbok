...
RFC 7522 Section 3 は、認可サーバーが受信した SAML アサーションに対して以下の検証を必須として規定する。仕様本文では 11 個の処理規則が番号付きで列挙されており、その要点を整理すると次のとおり。

1. アサーションは `<Issuer>` を持ち、値は発行者を一意に識別する
2. アサーションは `<Conditions>` を含み、`<AudienceRestriction>` の `<Audience>` 子要素として認可サーバーを示す URI を一つ以上指定しなければならない。認可サーバーは自身を識別する値を Simple String Comparison で比較し、含まれない場合はアサーションを拒否する
3. アサーションは `<Subject>` を含む。Authorization Grant では典型的にはリソースオーナーを、Client Authentication ではクライアント (`client_id`) を識別する
4. アサーションは利用可能な時間枠を制限する有効期限を持たなければならない
5. `<Subject>` には少なくとも一つの `<SubjectConfirmation>` が含まれ、その `Method` 属性は `urn:oasis:names:tc:SAML:2.0:cm:bearer` でなければならない (本仕様は **Bearer 確認のみ** を扱う)。また `<SubjectConfirmationData>` の `Recipient` 属性にはトークンエンドポイント URL を含めることが要求され、`NotOnOrAfter` で有効期限を、必要に応じて `Address` 属性で送信元を制限できる
6. `<Conditions>` の `NotOnOrAfter` を過ぎたアサーションを認可サーバーは拒否しなければならない。クロックスキューは認可サーバーのポリシーで許容される範囲内に収める
7. 主体が直接認証された場合は単一の `<AuthnStatement>` を含めるべきであり、クライアント自身が裁量で発行する場合 (autonomous client) は省略する
8. `<AttributeStatement>` その他のステートメントを含めてもよい
9. アサーションは発行者の鍵によりデジタル署名 (XML Signature) または MAC が付与されていなければならない。認可サーバーは署名検証に失敗したアサーションを拒否しなければならない
10. SAML 2.0 の仕様に従って暗号化された要素を含めてもよい
11. 認可サーバーは上記以外の観点でも SAML 2.0 の仕様に照らして無効なアサーションを拒否しなければならない

これらの要件は [RFC 7521](./rfc7521.md) が抽象的に定める検証要件を、SAML 2.0 固有の要素にマッピングしたものといえる。
...
RFC 7522 Section 6 (Security Considerations) および Section 7 (Privacy Considerations) は、[RFC 7521](./rfc7521.md) のセキュリティ考慮事項、[RFC 6749](./rfc6749.md) Section 10、および [OASIS SAML 2.0 のセキュリティ・プライバシー考慮事項 (SAML2.0-Security)](http://docs.oasis-open.org/security/saml/v2.0/saml-sec-consider-2.0-os.pdf) の議論を包括的に参照することを求める。特に押さえるべき要点は以下。
...
RFC 7522 における検証失敗時 (Section 3.1 / 3.2)、認可サーバーは [RFC 6749](./rfc6749.md) の標準エラーレスポンスを返す。
...
RFC 7522 Section 8 (IANA Considerations) は、RFC 6755 が定める OAuth URI レジストリに対し以下二つの URN を新規登録する。

- `urn:ietf:params:oauth:grant-type:saml2-bearer` (Section 8.1)
- `urn:ietf:params:oauth:client-assertion-type:saml2-bearer` (Section 8.2)
