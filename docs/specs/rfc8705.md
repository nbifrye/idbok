...
パブリッククライアントの場合、リフレッシュトークンも同じ証明書にバインドすることが推奨されます (機密クライアントの場合は、クライアント認証によって間接的にバインドされる)。
...
mTLS を利用するクライアントは、対応するエンドポイントの URL に `mtls_endpoint_aliases` 内のエイリアスを **優先して** 利用します。これにより、通常の OAuth クライアントが mTLS エンドポイントへアクセスして証明書選択ダイアログに困らされる事態を防げます。

### Section 6: 認可サーバー / クライアントメタデータの拡張
...
#### クライアントメタデータ (RFC 7591 / RFC 7592)

- `tls_client_auth_subject_dn`, `tls_client_auth_san_dns`, `tls_client_auth_san_uri`, `tls_client_auth_san_ip`, `tls_client_auth_san_email`: PKI 方式での照合対象。
- `tls_client_certificate_bound_access_tokens` (Boolean): クライアントが証明書バインドアクセストークンを希望するか。`true` の場合、クライアントはトークン取得を mTLS 経由で行うことを期待される。
