...
- 認可リクエストに含めた `resource` は、その認可グラント全体に適用される。認可サーバはローカルポリシーに基づき、後続のトークンリクエストで指定できる `resource` を当初要求された集合のサブセットに制限することがある。
...
- **`authorization_code` / `refresh_token`**: 認可サーバのローカルポリシーにより、受け入れる `resource` 値は当初付与されたリソース集合またはそのサブセットに限定され得る。`authorization_code` グラントで要求されたリソースが元のリソース集合のサブセットである場合、認可サーバはそのサブセットに基づくアクセストークンを発行し、返されるリフレッシュトークンは元の (完全な) グラントに紐付く。これにより、別の `resource` を指定して再度トークンを取得することが可能。
- **その他の grant type (`client_credentials` 等)**: 認可サーバのローカルポリシーに従って `resource` 値が解釈される (受理する値は認可サーバの裁量による)。

### 5.4 `resource` と `scope` の組み合わせ

両方が指定された場合のセマンティクスは、「要求されたすべてのスコープが、要求されたすべてのリソースで利用可能であること」を意味する。すなわち実効的なアクセス権は「全スコープ × 全ターゲットサービス」のデカルト積となる。仕様は次のように述べる。

- 認可サーバは可能な限り、各リソースが処理可能かつ必要とする値までスコープを「ダウンスコープ」してアクセストークンに紐付けるべき (should)。最終的なスコープが要求と異なる場合は、RFC 6749 Section 5.1 に従い、レスポンスの `scope` パラメータでクライアントに通知する。
- リソースとスコープの組み合わせが許容できない場合、認可サーバは `invalid_target` エラーで拒否することができる。
