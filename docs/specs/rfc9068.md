...
JWT の用途を示すヘッダ。本プロファイルでは発行時の値を `at+jwt` とすべきとされ、検証側は `at+jwt` または `application/at+jwt` を受け入れる。これにより ID Token (`typ` がない、または別の値) との混同を防ぐ。
...
- 発行時の `typ` ヘッダパラメータの値は `at+jwt` であるべき (SHOULD) とされる。一方、検証側は `at+jwt` または `application/at+jwt` のいずれかでなければ拒否しなければならない (MUST)。なお Section 3 の例では `at+JWT` のように大文字を含む値が示されており、両形式が許容される運用が想定されている。
...
- クライアントが `resource` パラメータ (RFC 8707) を含めた場合、認可サーバは `aud` クレームの値を `resource` パラメータと同じ値にすべきである (SHOULD)。
- `resource` パラメータがない場合、認可サーバは `aud` クレームに既定のリソース指標を用いなければならない (MUST)。リクエストに `scope` パラメータがある場合は、その値から既定のリソース指標を推定すべきとされる。
- 認可サーバは、付与される認可が曖昧になるような JWT アクセストークンを発行してはならない。複数の `scope` 値がそれぞれ異なる既定のリソース指標に対応する場合、認可サーバは `invalid_scope` エラーで要求を拒否すべきである。

### 5.7 検証手順 (Section 4)

リソースサーバは次の順で検証を実行する。失敗した場合は RFC 6750 に従い `invalid_token` エラーで応答する。

1. **`typ` 検証**: ヘッダの `typ` が `at+jwt` または `application/at+jwt` でなければ拒否する。
2. **復号 (必要な場合)**: JWE として暗号化されている場合は、リソースサーバ登録時に指定した鍵とアルゴリズムで復号する。
3. **`iss` 検証**: ディスカバリ等で取得した認可サーバの issuer 識別子と `iss` クレームの値が完全一致しなければならない。
4. **`aud` 検証**: `aud` クレームに自リソースサーバを指す識別子が含まれていなければならない。含まれない場合は拒否する。
5. **署名検証**: RFC 7515 に従い、JWT ヘッダの `alg` で示されるアルゴリズムで署名を検証する。
6. **`alg=none` の拒否**: `alg` の値が `none` の JWT は拒否しなければならない。
7. **`exp` 検証**: 現在時刻が `exp` クレームより前でなければならない。
8. **認可情報クレームの活用**: トークンに `scope` などの認可情報クレームが含まれる場合、リソースサーバは他のコンテキスト情報と組み合わせてアクセス制御に用いるべきである。
9. **エラーハンドリング**: いずれかの検証に失敗した場合、RFC 6750 Section 3.1 に従って `invalid_token` エラーで応答する。

クロックスキュー吸収のため `exp` 比較に数分程度の許容を設ける運用は一般的に行われる。実装によっては `iat` の妥当性や `jti` の重複利用検知も併せて行う。
